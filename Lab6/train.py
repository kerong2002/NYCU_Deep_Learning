...
# 從 diffusers 導入 DDPMScheduler 和學習率排程器
...
def train(args, device):
    ...
    # --- 1. 初始化設定 ---
    print("開始訓練模式 (CFG + LR Scheduler)...")
    # 專業命名：將輸出根目錄命名為 "output"，並在其中創建子目錄
        ...
        if (epoch + 1) % args.save_interval == 0:
            ...
            with torch.no_grad():
                ...
                for t in tqdm(noise_scheduler.timesteps):
                    t = t.to(device) # 確保時間步張量在正確的設備上
                    # 將純量 t 擴展為批次大小的張量，以匹配模型的輸入要求
                    t_for_model = t.expand(preview_images.shape[0])
...
def main():
    parser = argparse.ArgumentParser(description="訓練一個高效能的條件式 DDPM")
    ...
    parser.add_argument("--run_name", type=str, default="ddpm-cfg-v1", help="本次執行的名稱，用於 wandb 和輸出目錄")
    parser.add_argument("--project_name", type=str, default="Lab6-DDPM-Stronger", help="wandb 專案名稱")
    ...
    parser.add_argument("--num_train_timesteps", type=int, default=500, help="訓練時間步數 (增加步數以提升品質)")
