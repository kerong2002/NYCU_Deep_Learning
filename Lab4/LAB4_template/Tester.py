    ...
    def __init__(self, root, transform, mode="test", video_len=7, partial=1.0):
        ...
        for i in range(data_num):
            self.img_folder.append(
                sorted(glob(os.path.join(root, "test", "test_img", str(i), "*")), key=get_key)
            )
            self.label_folder.append(
                sorted(glob(os.path.join(root, "test", "test_label", str(i), "*")), key=get_key)
            )
...
class Test_model(VAE_Model):
    ...
    def make_gif(self, images_list, img_name):
        ...
        new_list[0].save(img_name, format="GIF", append_images=new_list, save_all=True, duration=20, loop=0)
